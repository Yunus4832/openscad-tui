...
- 存在一个底部的命令行，用于输入命令以及参数等；
- 存在一个预览窗口，展示生成的代码；
- 交互以命令行为中心所有的焦点都在命令行，所有的操作都可以通过命令实现，但是可以通过按键绑定快速执行命令；
- 提供一些内建的模块，例如 cube、square、等，这些 OpenSCAD 原生提供的基础模块；
- 使用类似 Vim 的交互方式有两种模式，Command 模式直接输入命令并执行，Noremal 模式可以使用按键绑定快速执行或触发命令
- jkhl 分别绑定到树上下移动，展开和收起节点的命令;
...
- : 绑定到 command 命令，进入命令模式，可以输入合法的命令，并在输入框回显 enter 执行命令
- y 绑定到 yank 命令，复制节点；
...
- e 绑定到 edit 命令，加载指定 json 文件，重建树形目录，继续编辑；
...
主要的实现逻辑的核心是 OpenSCAD 库定义文件，该文件也是一个自定义的 json 文件，需要库作者提供，或者用户解析库并
自行创建，它描述了
