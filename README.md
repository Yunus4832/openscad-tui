...
> 当前项目处于可运行的原型阶段，未发布版本号为 `0.0.1`，尚未发布到 crates.io；
> 可以通过本地源码或 Git 安装。
...
需要 Rust 2021 edition 兼容工具链。当前版本尚未发布到 crates.io，从已经拉取的源码目录
安装时在仓库根目录执行：
...
现代 Cargo 不再允许用裸 `cargo install` 安装当前目录中的 package，必须显式提供
`--path .`。

也可以直接从 GitHub 安装当前主分支：

```bash
cargo install --git https://github.com/Yunus4832/openscad-tui.git --locked openscad-tui
```

两种方式都会在 Cargo 的二进制目录中安装 `openscad-tui`；默认通常是 `~/.cargo/bin`。
如果该目录尚未加入 `PATH`，需要先按 Rust 工具链的安装提示配置环境变量。验证安装：

```bash
openscad-tui --version
```

卸载使用 `cargo uninstall openscad-tui`。源码编辑、项目读写、静态 OFF/STL/DAE 预览和
`.scad` 导出不依赖 OpenSCAD；执行 `model render` 或编译装配零件时仍要求 PATH 中存在
`openscad` 可执行文件。

## 构建与运行

OpenSCAD 调用的超时时间为 120 秒。
...
`source import existing.scad` 的导入行为相同。
...
| `j` / `k`、`↓` / `↑` | 移动树光标 |
| `h` / `l`、`←` / `→` | 折叠或展开节点 |
| `Enter` | 切换节点展开状态；打开选中的项目源文件或装配体 |
| `v` | 选择或取消选择当前节点 |
| `Space` | 显示或隐藏当前节点（存在多选时作用于所有选中节点） |
| `y` / `p` | 模块节点：复制／粘贴子树；项目源文件：复制／粘贴源码文件 |
...
| `n` | 打开 `source new`，新建项目内 SCAD 源文件 |
| `t` / `r` / `s` | 模块节点上打开平移、旋转或缩放命令；项目源文件上 `r` 为重命名 |
| `d` | 模块节点：剪切子树并移到最近兄弟节点；项目源文件：剪切到源码剪贴板 |
| `u` / `Ctrl+R` | 撤销或重做 |
| `P` / `R` | 切换源码/模型预览；强制重新渲染当前 buffer 并进入模型预览 |
| `w` | 保存 `.scadtui` 项目包；首次保存时等待输入项目路径 |
| `o` | 打开 `project open` 命令，加载 `.scadtui` 项目包 |
| `e` | 打开 `source import` 命令，解析 `.scad` 文件进行结构化编辑 |
| `L` | 打开 `library load` 命令，加载并嵌入 `.scad` 源码库 |
| `:` | 进入命令模式 |
| `?` | 显示帮助 |
| `q` / `Ctrl+C` | 退出 |
...
项目资源采用 `<资源> <动作>` 形式（如 `source new`、`model export`）；`insert`、`set`、
`translate` 等直接作用于当前 AST 选择的建模操作继续保持简短的动作式命令。
...
容器模块优先包装用 `v` 选中的节点；没有显式选择时会包装树光标所在的当前节点：
...
`linear_extrude` 和 `rotate_extrude` 是容器模块。可先用 `v` 选择一个或多个 2D 子节点；
没有显式选择时，它们默认使用当前 2D 节点，不会仅因选择列表为空而报错。
...
参数提示所需的常用内建签名，并不等同于 OpenSCAD 的完整标准库；签名表仍可能有未覆盖
的内建模块，外部 SCAD 库则通过 `library load` 加载。
...
```text
project new vernier
project rename "vernier caliper"
project open project.scadtui
project save
project save project.scadtui
project save-as project.scadtui --force
project export-sources ./source-tree
source new part
source import existing.scad
source list
source switch vernier_cursor.scad
source next
source rename part.scad jaw
source remove unused.scad
source copy part.scad
source cut unused.scad
source paste copied-part
source export model.scad
library load gears.scad
library list
library remove gears.scad
source use gears.scad
source include gears.scad
model render
model preview --render
model export model.stl
model export model.dae
model view model.off
model view model.stl
assembly new robot
...
- `project save [path]` 保存可继续编辑的 `.scadtui` 项目包，首次保存可以直接给出路径；
  `project open` 读取项目包。
  `project save-as` 选择新的包路径，`project rename` 只改变包内项目名称。项目包是 ZIP 容器，
  包含版本化 `manifest.json`、结构化项目数据和可检查的 `sources/` SCAD 快照。
  首次保存未命名项目时，项目标题从包文件名推导；打开名称仍为 `untitled` 的项目包时也会
  使用包文件名显示标题。
- `project new [name] [--force]` 创建包含空白 `main.scad` 的项目；项目路径可由首次
  `project save <path>` 指定。默认项目名会从第一次保存的文件名推导。`source new part`
  在当前项目中新建 `part.scad`，省略后缀时默认
  使用 `.scad`。
- `source import` 将已有 `.scad` 文件及其本地依赖导入当前项目，连续执行会归集为多个可编辑
  buffer，不会替换已有 source。导入后内容与原文件解耦。
- `source import` 会递归收集能从项目目录或 OpenSCAD 库目录解析到的 `include` / `use` 文件，将
...
- 项目源文件节点采用独立的上下文按键：`r` 重命名，`y` 复制到源码剪贴板，`d` 剪切，
  `p` 粘贴。粘贴副本使用稳定的数字后缀，例如 `arm.scad`、`arm2.scad`、`arm3.scad`。
  复制或剪切会保留该 source 发出的依赖关系；粘贴到不同目录时会重新计算并改写相对
  `use` / `include` 引用。`source remove` 仍是不会写入剪贴板的永久移除命令。
- `source list/switch/next/previous` 列出或切换可编辑源文件，文件名、文件 stem 和项目内相对路径
  都可使用。切换前会把当前 AST 写回其嵌入记录，切换后模块和函数补全按新文件的可达依赖重建。
- `source rename` 会同时更新项目依赖、`use` / `include` 路径和装配 mesh 引用；
  `source remove` 只允许删除未被其他源码或装配引用的 source，并保证项目至少保留一个可编辑 source。
- `model render` 直接渲染当前编辑文件。渲染时所有项目文件都会从各自 AST 生成到临时目录，
  外部只读库则恢复导入时的原文，再由 OpenSCAD 按原有 `include` / `use` 关系处理。
- `source import` 后可使用 `project save project.scadtui` 首次保存项目；不会覆盖原始 `.scad` 文件。
  `project open/new ... --force` 可以明确丢弃未保存状态。
- `source export <file.scad>` 只导出当前 buffer；`project export-sources <directory>` 导出当前 buffer
  及其可达依赖组成的完整 SCAD 源码树；`model export <artifact>` 按目标后缀生成模型产物。
...
- `model view <model.off|model.stl|scene.dae>` 直接加载现有 OFF、ASCII/Binary STL 或静态 DAE
  白膜并进入模型预览，不修改当前项目，也不调用 OpenSCAD。DAE 预览读取多个 mesh、节点层级、
  实例变换和坐标轴，忽略材质；骨骼、控制器和动画不属于这一轻量预览范围。三种格式都可以
  直接作为启动参数。
- `library load gears.scad` 加载 OpenSCAD 源码库并递归收集本地 SCAD 依赖，但不会修改
  当前 source 的语义。源码会直接嵌入项目包，不需要额外的库描述文件。`library list`
  列出已加载的库根；`library remove` 删除尚未被任何项目 source 引用的库及其私有依赖树。
- `source use <source>` / `source include <source>` 在当前 buffer 与项目内另一个 source 之间建立对应
  关系；目标既可以是 `library load` 加载的只读库，也可以是 `source import/new` 创建的可编辑
...
- 编辑：`insert`、`insert-before`、`set`、`unset`、`delete`、`undo`、`redo`
...
- 项目：`project new|open|save|save-as|rename|export-sources`
- 源码：`source new|import|list|switch|next|previous|rename|remove|copy|cut|paste|use|include|export|preview`
...
执行 `model render` 会调用本机 `openscad`，把当前模型编译为内部三角网格；`model view` 则直接将
OFF/STL 文件加载为单实例场景，或把静态 DAE 加载为多个共享 mesh、节点和实例矩阵。
装配预览也直接提供多实例场景。这些路径共用 `openscad-render` 的 `RenderScene` 后台 CPU 光栅化，
...
也可以通过 `display protocol` 命令切换后端；缓存的 RGBA 帧会立即提交给新后端，无需重新
...
display protocol auto|next|kitty|sixel|iterm2|halfblocks|braille|ascii
diagnostics [file]
```

Editor、Model 和 Assembly Screen 的普通模式快捷键统一由上下文 keymap 解析，并最终执行已注册
Command。鼠标手势和底部按钮同样执行这些 `model`、`camera`、`display` 命令；工具栏按钮直接
从 keymap 的绑定元数据生成，不再维护另一份映射。
模型预览默认显示深度感知的世界坐标轴：正向 X/Y/Z 分别使用红、绿、蓝色，负向使用
对应的暗色。`display axes` 只会用缓存网格重新光栅化，不会再次调用 OpenSCAD；所有终端协议
共享同一张包含坐标轴的 RGBA 帧。
...
`model preview` 在尚无模型预览时会自动执行一次渲染；已有预览时只切换 Screen，
不会强制重新运行 OpenSCAD。
`model preview --render` 会强制重新渲染后进入预览。

进入模型预览时会切换到独立的 Model Preview Screen：`h/j/k/l` 环绕，方向键平移，`+/-` 缩放，
`R` 强制重新渲染当前 buffer，`f` 适配模型，`p` 切换投影，`x` 切换坐标轴，
`1..7` 选择标准视角，空格切换自动旋转，
`Esc/q` 关闭模型预览。项目内预览会返回 Source；以 `.off` / `.stl` / `.dae` 启动的独立模型查看
会直接退出程序。
在 Editor 中按 `P` 打开模型预览；Model Preview 中按 `P` 的关闭语义与 `Esc/q` 相同。
按 `R` 会执行 `model render` 命令，忽略已有预览缓存，重新渲染当前 buffer 并进入模型预览。
Model Preview Screen 同样可以按 `:` 进入 Command Mode，支持命令历史、Tab 补全和
直接执行 `camera`、`model`、`display` 等命令。

鼠标操作会根据所在面板分发：节点树中单击定位（再次点击展开/折叠）、Ctrl+单击
切换多选、滚轮滚动；源码预览中滚轮滚动代码；模型预览中左键拖动旋转、
Shift+左键拖动或右键拖动平移、滚轮缩放。

模型预览使用全宽布局并隐藏节点树。底部工具栏将快捷键说明置于上方，可点击按钮置于下方；
按钮不再重复显示快捷键，因此布局更加紧凑。首项会根据启动方式显示 Source 或 Quit，其余按钮
可以用鼠标执行 Fit、投影切换、坐标轴、Front、Top、Iso 和自动旋转等常用操作。
...
- SCAD 渲染和装配零件编译依赖外部 `openscad` 可执行文件；直接查看 OFF/STL/DAE 不依赖它
...
- Assembly 当前只支持刚性白膜零件、父子层级、TRS/pivot 和可见性；不提供顶点编辑、
  UV/材质、骨骼、蒙皮、动画或物理。静态 DAE 可以独立预览，但不会导入 Assembly 数据模型
- 不是自由文本源码编辑器，主要通过 AST 树和命令编辑
- 函数参数默认值、语义类型检查和高级补全仍有限
- README 描述的是当前代码状态；项目可从源码或 Git 安装，但尚未发布稳定安装包或 crates.io 版本
