...
- `project save` 保存可继续编辑的 `.scadtui` 项目包，`project open` 读取项目包。
  `project save-as` 选择新的包路径，`project rename` 只改变包内项目名称。项目包是 ZIP 容器，
  包含版本化 `manifest.json`、结构化项目数据和可检查的 `sources/` SCAD 快照。
- `project new [name] [--force]` 创建包含空白 `main.scad` 的项目；项目路径之后由
  `project save-as` 指定。`source new part` 在当前项目中新建 `part.scad`，省略后缀时默认
  使用 `.scad`。
...
- `source import` 后需使用 `project save-as project.scadtui` 保存项目；不会覆盖原始 `.scad` 文件。
...
- 源码：`source new|import|list|switch|next|previous|rename|remove|use|include|export|preview`
...
`assembly`、`camera`、`display axes`、`display protocol` 命令执行。复制实例会继承最初的命名基底，
...
Model Screen 的键盘映射、鼠标手势和底部按钮都是这些 `model`、`camera`、`display` 命令的
