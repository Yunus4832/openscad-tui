...
`.scad` 导出不依赖 OpenSCAD；执行 `render` 或编译装配零件时仍要求 PATH 中存在
`openscad` 可执行文件。
...
- `view <model.off|model.stl|scene.dae>` 直接加载现有 OFF、ASCII/Binary STL 或静态 DAE
...
`assembly`、`camera`、`axes`、`protocol` 命令执行。复制实例会继承最初的命名基底，
